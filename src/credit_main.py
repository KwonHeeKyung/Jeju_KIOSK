...
def ck_check():
    ...
    insert_data = json.loads(
        insert_ck.text[insert_ck.text.index('(') + 1: insert_ck.text.rindex(')')].replace("'", '"'))
...
def tokenRes():
    tokenRes = requests.post(
        'http://localhost:8090/?callback=jsonp12345678983543344&REQ=TR^^F^^^^^^^^^30^^^^TK^^^^^^^^^^^^^^^^^')
    ...
    if token_data["SUC"] == '00' and token_data['RS04'] == '0000' and token_data["RS31"].strip() == 'C':
        if token_data['RS11'] == '027' or token_data['RS11'] == '006' or token_data['RS11'] == '008':
            ...
            if token_data['RS34'] == 'N':
                ...
                provis_data = json.loads(
                    provisRes.text[provisRes.text.index('(') + 1: provisRes.text.rindex(')')].replace("'", '"'))
                if provis_data['RS04'] == '0000':
                    rd.set('ap', provis_data['RS09'])
                    rd.set('cd', provis_data['RS08'])
                    rd.set('refund_time', provis_data["RS07"][0:6])
                    rd.set('msg', 'remove')
                    rd.set('method', 'check')
                    cancel_text = f'http://localhost:8090/?callback=jsonp12345678983543344&REQ=D4^^30000^^{provis_data["RS07"][0:6]}^{provis_data["RS09"]}^^^^^^20^A^^^^^^^{provis_data["RS08"]}^^^^^^^^^^^^^^^^^'
...
def cancelProvis():
    if method == b'check':
        ...
        refund_time = rd.get('refund_time').decode('utf-8')
        ...
        cancelRes_text = json.loads(
            cancelRes.text[cancelRes.text.index('(') + 1: cancelRes.text.rindex(')')].replace("'", '"'))
...
def payment():
    ...
    if int(total_price) > 0:
        ...
        if pay_data['RS04'] == '0000':
            rd.set('msg', 'end')
            refund_param = f"http://localhost:8090/?callback=jsonp12345678983543344&REQ=D4^^{total_price}^^{pay_data['RS07'][0:6]}^{pay_data['RS09']}^^^^^^20^A^^^^^^^{pay_data['RS08']}^^^^^^^^^^^^^^^^^"
